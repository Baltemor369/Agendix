...
# Liste des dépôts
st.subheader("📋 Liste des dépôts")
depots = get_depots()
if depots:
    for depot in depots:
        st.write(
            f"Depot {depot[0]} : {depot[1]} - {depot[2]} {depot[3]}, {depot[4]} {depot[5]} "
...
with st.form("add_depot_form"):
    ...
    if submitted:
        ...
        if lat and lon:
            ...
            st.session_state["message"]=("✅ Dépôt ajouté avec succès !",1)
            st.rerun()
        else:
            st.error("❌ Impossible de géocoder l'adresse.")

# Modifier un dépôt
st.subheader("✏️ Modifier un dépôt")
if depots:
    # Crée une liste des noms de dépôts
    nom_depots = [d[1] for d in depots]

    # Sélection par nom
    nom_selectionne = st.selectbox("Choisir un dépôt à modifier", nom_depots)
    ...
    with st.form("update_depot_form"):
        ...
        if submitted:
            ...
            if lat and lon:
                ...
                st.session_state["message"]=("✅ Dépôt modifié avec succès !",1)
                st.rerun()
            else:
                st.error("❌ Impossible de géocoder l’adresse.")

# Supprimer un dépôt
st.subheader("🗑️ Supprimer un dépôt")
if depots:
    nom_depots = [d[1] for d in depots]
    nom_a_supprimer = st.selectbox("Choisir un dépôt à supprimer", nom_depots, key="delete_select")
